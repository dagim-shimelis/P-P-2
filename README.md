...
Paste `scripts/check-portfolio.js` into Chrome DevTools' console on `/`, `/projects/list`, and `/projects/hahujobs-primary` to check content, navigation, artwork, filters, project grid layout, and image loading. Repeat at desktop and mobile widths. Test the menu's Escape key and focus return manually. Motion respects `prefers-reduced-motion`; the footer's color and grid controls also support `C` and `Cmd/Ctrl+G`.
